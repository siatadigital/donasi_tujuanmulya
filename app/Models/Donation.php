...
class Donation extends Model
{
    ...
    {
        return $query->where(\DB::raw('donations.status'), 'success');
    }

    public function scopePending($query)
    {
        $now = date('Y-m-d H:i:s');
        return $query->where(\DB::raw('donations.status'), 'pending');
    }

    public function scopeExpired($query)
    {
        $now = date('Y-m-d H:i:s');
        return $query->where(\DB::raw('donations.status'), 'expired');
    }

    public function scopeFailed($query)
    {
        return $query->where(\DB::raw('donations.status'), 'failed');
    }
}