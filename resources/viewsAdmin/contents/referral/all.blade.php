...
@section('content')
	...
	<div class="nav-tabs-custom">
		...
		<div class="tab-content">
				@if(Route::current()->getName() == "admin.referral.getAll")
					<form action="{{ route("admin.referral.getAllExport") }}" method="post">
				@endif
				{{ csrf_field() }}
				<div class="row">
              ...
              <option value="Pilih Tipe Cari">Pilih Tipe Cari</option>
				...
				<div class="row">
					<div class="col-md-4">
						...
						@if(Route::current()->getName() == "admin.referral.getAll")
							@if (isPermitted('admin.referral.getAllExport'))
			...
			<h2 class="pull-left">Total Semua Transaksi {{ priceFormat($total) }}</h2>
			<br><br><br><hr>
        ...
        <tbody>
          @foreach($items as $item)
					<tr>
						...
						<td>
							<strong>{{ $item['akad'] }}</strong>
							@if ($item['akad'] === 'Infak Terikat')
								<p>Judul Campaign: {{ $item['project_title'] }}</p>
							@endif
							@if ($item['akad'] === 'Zakat')
								<p>Jenis Zakat: {{ $item['type'] }}</p>
							@endif
						</td>
