...
@section('content')
	...
	<div class="box box-default">
		<div class="box-body">
			<form method="post" action="{{ url('/backend/group_privilege/update/'.$data->id) }}">
				{{ csrf_field() }}
				<div class="col-md-12">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="col-md-12">
                                @foreach($detail as $item2)
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label><input type="checkbox" name="menu1[]" value="{{ $item2->menu_admin_id }}" checked> {{ $item2->menuAdmin->title }} </label>
                                        </div>
                                    </div>
                                @endforeach
                                @foreach ($menu_admin as $item)
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label><input type="checkbox" name="menu2[]" value="{{ $item->id }}"> {{ $item->title }} </label>
                                        </div>
                                    </div>
                                @endforeach        
                            </div>
