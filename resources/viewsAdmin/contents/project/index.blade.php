			...
			<table class="table table-bordered table-striped table-hover">
				...
				@foreach($projects as $pr)
					<tr>
						...
						<td>{{ $pr['status'] }}</td>
						<td>{{ $pr['is_featured'] == "0" ? "No" : "Yes" }}</td>
						<td>{{ formatTime($pr['created_at']) }}</td>
						<td>
							@if (isPermitted('project.getEdit'))
							<a href="{{ route('project.getEdit', $pr['slug']) }}" class="btn btn-warning btn-sm">
							...
							@endif
							@if (isPermitted('admin.project.changeFeatured'))
								@if ($pr['is_featured'] == "0")
								<a href="{{ route('admin.project.changeFeatured', $pr['id']) }}" class="btn btn-primary btn-sm">
									<i class="fa fa-search"></i>
									Featured
								</a>
								@else
								<a href="{{ route('admin.project.changeFeatured', $pr['id']) }}" class="btn btn-warning btn-sm">
									<i class="fa fa-search"></i>
									Not Featured
								</a>
								@endif
