...
@section('title', 'RTL - Penjualan Offline')
...
<div class="section-header">
  ...
  <div class="section-header-breadcrumb">
    ...
    <div class="breadcrumb-item">Penjualan Offline</div>
  </div>
</div>

<div class="section-body">
  <h2 class="section-title">Penjualan Offline</h2>
  <p class="section-lead">
    Daftar penjualan offline
  </p>
      ...
      <div class="card">
        <form>
          <div class="card-header">
            <h4>Penjualan Offline</h4>
          </div>
          <div class="card-body">
            @if(count($stockTypesAllowed) > 0)
            <a href="{{ route('admin.offline-sales.create') }}" class="btn btn-primary">Tambah</a>
            @endif
            @if ($statusId == 3)
                <a href="{{ route('admin.offline-sales.export_sudahbayar') }}" class="btn btn-success float-right">Export ke Excel</a>
            @endif
            <br><br>
            <ul class="nav nav-tabs" id="tab" role="tablist">
                ...
                <li class="nav-item">
                    ...
                    <a
                        ...
                        id="{{ $tab->slug }}-tab"
                        href="{{ route('admin.offline-sales.index') . $params }}"
                        data-index="{{ $index }}"
...
<script>
  ...
  var table = $(tableId).DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.datatables.offline-sales", ['statusId' => '']) }}/' + statusId,
        columns: [{
            data: 'id',
            name: 'id'
        },
        ...
        columnDefs: [
            {
                ...
                render: function(data, type, row)  {
                    ...
                    var statusSelection = `
                        <form action="{{ route('admin.offline-sales.status.update', ['id' => '']) }}/${row.id}" method="POST">
                            {{ csrf_field() }}
                            {{ method_field('PUT') }}
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    var edit = '<a href="{{ route("admin.offline-sales.edit", ["id" => "ID_HERE"]) }}" class="btn btn-warning">Ubah</a>';
                    var printSales = '<a target="_blank" href="{{ route("admin.offline-sales.print", ["id" => "ID_HERE"]) }}" class="btn btn-success">Print Nota</a>';
                    ...
                    var isEditPermitted = Number("{{ (int) isPermitted('admin.offline-sales.edit') }}");
                    var isPrintSalesPermitted = Number("{{ (int) isPermitted('admin.offline-sales.print') }}");
  ...
  $('table').on('click', '.btn-detail', function() {
    ...
    $.ajax({
        url: "{{ route('admin.offline-sales.show', ['id' => '']) }}/" + itemId,
        method: 'GET',
        dataType: 'json',
        success: function(response) {
            ...
            var courierCost = item.delivery ? item.delivery.courier_cost : 0;
            ...
            $('#modal-detail #status-form').attr('action', "{{ route('admin.offline-sales.status.update', ['id' => '']) }}" + '/' + itemId);
            ...
            $('.detail-content.customer').text(item.delivery ? item.delivery.destination_fullname : item.customer ? item.customer.user.fullname : 'Guest');
            $('.detail-content.stock-type').text(item.stock_type === 'warehouse' ? 'Gudang' : 'Toko');
            ...
            var cancelUrl = "{{ route('admin.offline-sales.cancel', ['id' => 'ID_HERE']) }}".replace('ID_HERE', itemId);
  ...
  $('table').on('change', 'select', function() {
      ...
      $.ajax({
          ...
          success: function(response) {
              ...
              iziToast.success({
                  title: 'Berhasil!',
                  message: 'Berhasil mengubah status penjualan offline',
                  position: 'topRight'
              });
          ...
          error: function(e) {
              iziToast.error({
                  title: 'Gagal!',
                  message: 'Gagal mengubah status penjualan offline',
                  position: 'topRight'
              });
