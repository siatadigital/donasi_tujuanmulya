...
@section('title', 'RTL - Tambah Penjualan Offline')
...
<div class="section-header">
  ...
  <div class="section-header-breadcrumb">
    ...
    <div class="breadcrumb-item active"><a href="{{ route('admin.offline-sales.index') }}">Penjualan Offline</a></div>
    <div class="breadcrumb-item">Tambah Penjualan Offline</div>
  </div>
</div>

<div class="section-body">
  <!-- <h2 class="section-title">Tambah Penjualan Offline</h2>
  <p class="section-lead">
    Form untuk tambah penjualan offline
  </p> -->
      ...
      <div class="col-md-6" id="information-data">
        <div class="card">
          <div class="card-header">
            <h4>(1.) Informasi Penjualan Offline</h4>
          </div>
          <div class="card-body">
            <div class="row">
                <div class="col-md-6">
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-3" id="total-items-column" style="display:none;">
                    @component('admin.components.form-input', [
                        'label' => 'Jumlah Item',
                        'type' => 'number',
                        'name' => 'total_items',
...
var $totalItems = $('input[name=total_items]');
var $btnAdd = $('#btn-add');
...
$products.on('change', function() {
    ...
    var stockType = $('select[name=stock_type]').val();
    var totalItems = $('input[name=total_items]').val();
        ...
        success: function(response) {
            ...
            if (buyType === 'seri') {
                ...
                if (isShoes) {
                    $.ajax({
                        method: "GET",
                        url: "{{ route('admin.api.shoes-colors') }}",
                        data: {
                        ...
                        success: function(response) {
                            ...
                            var shoesColors = response.data;
                            var colorKey = shoesColors[0].id;

                            $('#row-shoes-colors').show();
                            $shoesColors.empty();

                            for (var color of shoesColors) {
                                $shoesColors.append(`
                            ...
                            $.ajax({
                                method: "GET",
                                url: "{{ route('admin.api.pick-seri-colors') }}",
                                data: {
                                    product_id: productId,
                                    stock_type: stockType,
                                    bought_items: items.filter(item => item.productId == productId),
                                    times: totalItems,
                                    color: colorKey,
                                },
                                success: function(response) {
                                    ...
                                    for (var colors of seriColors) {
                                        var selectedColors = colors.map(color => `
                        ...
                        error: function(error) {
                            ...
                            iziToast.error({
                                title: 'Gagal!',
                                message: 'Tidak dapat mengambil warna sepatu',
                                position: 'topRight'
                            });
                        }
                    });
...
$totalItems.on('change', function() {
    ...
    var productId = $products.val();
    var stockType = $('select[name=stock_type]').val();
    var colorKey = isShoes ? $shoesColors.val() : undefined;
...
$btnAdd.on('click', function() {
    ...
    if (buyType === 'seri') {
        ...
        for (const itemColors of colors) {
            var newItem = Object.assign({
                id: Date.now(),
                type: 'seri',
                currentPrice: productInfo.priceSellSeri,
                priceUsed: discountedPrice,
                colors: itemColors.map(color => {
