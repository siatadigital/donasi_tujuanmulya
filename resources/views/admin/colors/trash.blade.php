...
@section('title', 'RTL - Sampah Warna')
...
<div class="section-header">
  ...
  <div class="section-header-breadcrumb">
    ...
    <div class="breadcrumb-item">Sampah Warna</div>
  </div>
</div>

<div class="section-body">
  <h2 class="section-title">Sampah Warna</h2>
  ...
  <div class="row">
    <div class="col-12 col-md-12 col-lg-12">
      <div class="card">
        ...
        <div class="card-body">
          <form action="{{ route('admin.colors.trash.deleteAll') }}" method="POST" id="form-delete-all">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
...
var table = $('#table').DataTable({
  processing: true,
  serverSide: true,
  ajax: '{{ route("admin.datatables.trashes", ["type" => "colors"]) }}',
  columns: [{
      data: 'id',
      name: 'id'
    },
  ...
  columnDefs: [{
    ...
    render: function(data, type, row) {
      var edit =
        '<a href="{{ route("admin.colors.trash.restore", ["id" => "ID_HERE"]) }}" class="btn btn-warning btn-restore">Pulihkan</a>';
      var remove = '<button class="btn btn-danger btn-delete">Hapus</button>';
      var form =
        '<form action="{{ route("admin.colors.trash.delete", ["id" => "ID_HERE"]) }}" method="POST" class="form-inline form-delete">{{ method_field("DELETE") }}{{ csrf_field() }}' +
        edit + '&nbsp;&nbsp;' + remove + '</form>';
