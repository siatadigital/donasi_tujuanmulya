...
@section('title', 'RTL - Sampah Produk')
...
<div class="section-body">
  <h2 class="section-title">Sampah Produk</h2>
  <p class="section-lead">
    Daftar sampah produk
  </p>
  ...
  <div class="row">
    <div class="col-12 col-md-12 col-lg-12">
      <div class="card">
        ...
        <div class="card-body">
          <form action="{{ route('admin.products.trash.deleteAll') }}" method="POST" id="form-delete-all">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
...
var table = $('#table').DataTable({
  processing: true,
  serverSide: true,
  ajax: '{{ route("admin.datatables.trashes", ["type" => "products"]) }}',
  columns: [{
      data: 'id',
      name: 'id'
    },
  ...
  columnDefs: [
    {
        ...
        render: function(data, type, row) {
            var stocks = `
                Gudang (${row.current_total_stock_warehouse})<br/>
                Toko (${row.current_total_stock_store})<br/>
    ...
    },
    {
        ...
        render: function(data, type, row) {
            var edit =
                '<a href="{{ route("admin.products.trash.restore", ["id" => "ID_HERE"]) }}" class="btn btn-warning btn-restore">Pulihkan</a>';
            var remove = '<button class="btn btn-danger btn-delete">Hapus</button>';
            var form =
                '<form action="{{ route("admin.products.trash.delete", ["id" => "ID_HERE"]) }}" method="POST" class="form-inline form-delete">{{ method_field("DELETE") }}{{ csrf_field() }}' +
                edit + '&nbsp;&nbsp;' + remove + '</form>';
