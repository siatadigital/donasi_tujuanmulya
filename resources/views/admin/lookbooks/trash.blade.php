...
@section('title', 'RTL - Sampah Lookbook')

@section('content')
<div class="section-header">
  <h1>Sampah Lookbook</h1>
  <div class="section-header-breadcrumb">
    <div class="breadcrumb-item active"><a href="{{ route('admin.dashboard.index') }}">Dashboard</a></div>
    <div class="breadcrumb-item active"><a href="{{ route('admin.lookbooks.index') }}">Lookbook</a></div>
    <div class="breadcrumb-item">Sampah Lookbook</div>
  </div>
</div>

<div class="section-body">
  <h2 class="section-title">Sampah Lookbook</h2>
  <p class="section-lead">
    Daftar sampah lookbook
  </p>
    ...
    <div class="col-12 col-md-12 col-lg-12">
      <div class="card">
        <div class="card-header">
          <h4>Sampah Lookbook</h4>
        </div>
        <div class="card-body">
          <form action="{{ route('admin.lookbooks.trash.deleteAll') }}" method="POST" id="form-delete-all">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
...
var table = $('#table').DataTable({
  processing: true,
  serverSide: true,
  ajax: '{{ route("admin.datatables.trashes", ["type" => "lookbooks"]) }}',
  columns: [{
      data: 'id',
      name: 'id'
    },
  ...
  columnDefs: [{
    ...
    render: function(data, type, row) {
      var edit =
        '<a href="{{ route("admin.lookbooks.trash.restore", ["id" => "ID_HERE"]) }}" class="btn btn-warning btn-restore">Pulihkan</a>';
      var remove = '<button class="btn btn-danger btn-delete">Hapus</button>';
      var form =
        '<form action="{{ route("admin.lookbooks.trash.delete", ["id" => "ID_HERE"]) }}" method="POST" class="form-inline form-delete">{{ method_field("DELETE") }}{{ csrf_field() }}' +
        edit + '&nbsp;&nbsp;' + remove + '</form>';
