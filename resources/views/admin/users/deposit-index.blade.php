...
<div class="section-body">
  ...
  <p class="section-lead">
    Daftar deposit milik <strong>{{ $user->fullname !== "" ? $user->fullname : $user->username  }}</strong>, total saldo deposit saat ini <strong>{{ 'Rp. ' . number_format($user->customer ? $user->customer->current_deposit_amount : 0) }}</strong>
  </p>
    ...
    <div class="col-12 col-md-12 col-lg-12">
      <div class="card">
        <form>
          ...
          <div class="card-body">
            <a href="{{ route('admin.users.deposits.create', ['id' => $id]) }}" class="btn btn-primary">Tambah</a>
            <br><br>
...
<script>
  var table = $('#table').DataTable({
    processing: true,
    serverSide: true,
    ajax: '{{ route("admin.datatables.user-deposits", ["id" => $id]) }}',
    columns: [{
        data: 'id',
        name: 'id'
      },
      ...
      render: function(data, type, row) {
        var show = `
            <button
                ...
                data-item-id="${row.accounting_id}"
            >
                Detail
            </button>
        `;
...
$('table').on('click', '.btn-detail', function() {
  ...
  $.ajax({
      ...
      success: function(response) {
          ...
          $('.detail-content.customer').text(item.customer_deposit.user.username);
          $('.detail-content.bank').text(item.customer_deposit.bank.bank_name + ' - ' + item.customer_deposit.bank.account_number + ' a.n. ' + item.customer_deposit.bank.account_name);
