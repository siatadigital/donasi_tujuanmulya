...
@section('title', 'RTL - Sampah Customer')

@section('content')
<div class="section-header">
  <h1>Sampah Customer</h1>
  <div class="section-header-breadcrumb">
    <div class="breadcrumb-item active"><a href="{{ route('admin.dashboard.index') }}">Dashboard</a></div>
    <div class="breadcrumb-item active"><a href="{{ route('admin.customers.index') }}">Customer</a></div>
    <div class="breadcrumb-item">Sampah Customer</div>
  </div>
</div>

<div class="section-body">
  <h2 class="section-title">Sampah Customer</h2>
  <p class="section-lead">
    Daftar sampah customer
  </p>
    ...
    <div class="col-12 col-md-12 col-lg-12">
      <div class="card">
        <div class="card-header">
          <h4>Sampah Customer</h4>
        </div>
        <div class="card-body">
          <form action="{{ route('admin.customers.trash.deleteAll') }}" method="POST" id="form-delete-all">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
...
var table = $('#table').DataTable({
  processing: true,
  serverSide: true,
  ajax: '{{ route("admin.datatables.trashes", ["type" => "customers"]) }}',
  columns: [{
      data: 'id',
      name: 'id'
    },
  ...
  columnDefs: [{
    ...
    render: function(data, type, row) {
      var edit =
        '<a href="{{ route("admin.customers.trash.restore", ["id" => "ID_HERE"]) }}" class="btn btn-warning btn-restore">Pulihkan</a>';
      var remove = '<button class="btn btn-danger btn-delete">Hapus</button>';
      var form =
        '<form action="{{ route("admin.customers.trash.delete", ["id" => "ID_HERE"]) }}" method="POST" class="form-inline form-delete">{{ method_field("DELETE") }}{{ csrf_field() }}' +
        edit + '&nbsp;&nbsp;' + remove + '</form>';
